...
use crate::{
    drivers::NET_DEVICE,
    net::socket::{Protocol, get_socket, push_data},
    sync::UPIntrFreeCell,
};
...
pub fn net_interrupt_handler() {
    ...
    match packet {
        ...
        Packet::UDP(udp_packet) => {
            ...
            if let Some(socket_index) = get_socket(target, lport, rport, Protocol::UDP) {
        ...
        Packet::TCP(tcp_packet) => {
            ...
            if let Some(socket_index) = get_socket(target, lport, rport, Protocol::TCP) {

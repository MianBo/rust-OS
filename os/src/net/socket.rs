...
pub fn get_socket(raddr: IPv4, lport: u16, rport: u16, protocol: Protocol) -> Option<usize> {
    let socket_table = SOCKET_TABLE.exclusive_access();
    for i in 0..socket_table.len() {
        ...
        if sock.raddr == raddr && sock.lport == lport && sock.rport == rport && sock.protocol == protocol{
            return Some(i);
        }
    }
    None
...
pub fn add_socket(raddr: IPv4, lport: u16, rport: u16, protocol: Protocol) -> Option<usize> {
    if get_socket(raddr, lport, rport, protocol).is_some() {
        return None;
    }
